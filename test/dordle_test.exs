defmodule DordleTest do
  ...
  test "find_exact_matches" do
    ...
    exact_locs = Dordle.find_exact_matches(locs_to_check, ~c"ZZZZZ", word_cl)
    assert exact_locs == []

    exact_locs = Dordle.find_exact_matches(locs_to_check, ~c"ZZZZE", word_cl)
    assert exact_locs == [5]

    exact_locs = Dordle.find_exact_matches(locs_to_check, ~c"ZRZZE", word_cl)
  ...
  test "output" do
    ...
    Dordle.output(~C"SHEAR", guesses, false)
  end
  ...
  test "create_rows_chars_to_which" do
    guesses = [
      [guess: "ARISE", exact_locs: [5], other_locs: [1, 4]],
      [guess: "SEATS", exact_locs: [1, 3, 4], other_locs: [2]]
    ]

    actual = Dordle.create_rows_chars_to_which(guesses)

    expected = [
      [{65, :other}, {82, :none}, {73, :none}, {83, :other}, {69, :exact}],
      [{83, :exact}, {69, :other}, {65, :exact}, {84, :exact}, {83, :none}]
    ]
